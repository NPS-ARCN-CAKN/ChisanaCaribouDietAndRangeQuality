...
knitr::kable(sqldf("SELECT Ecotype,[Mean lichen thickness (cm)],Min,Max,SD,n,[Number of plots] FROM Biomass ORDER BY [Mean lichen thickness (cm)] DESC"), digits = 2)
...
ggplot(Biomass) +
  ...
  geom_errorbar(aes(x=Ecotype,ymin=ifelse(`Mean lichen thickness (cm)` - SD < 0,0,`Mean lichen thickness (cm)` - SD),ymax=`Mean lichen thickness (cm)` + SD)) +
  geom_text(aes(x=Ecotype,y=`Mean lichen thickness (cm)`,label=round(`Mean lichen thickness (cm)`,digits=2)) ) +
  geom_text(aes(x=Ecotype,y=`Mean lichen thickness (cm)` + .5,label=EcotypeID ) ) +
  geom_text(aes(x=Ecotype,y=`Mean lichen thickness (cm)` + .8,label=paste("(",`Number of plots`,")",sep="") ) ) +
  theme_minimal() +
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) 

```

# KABLE IN THE BIOMASS DATA TABLE HERE AND FINISH THE PLOT
...
# Output the lichen thickness and biomass stats
knitr::kable(sqldf("SELECT Ecotype
...
,n,[Number of plots]
FROM Biomass ORDER BY [Mean lichen thickness (cm)] DESC"), digits = 2)
