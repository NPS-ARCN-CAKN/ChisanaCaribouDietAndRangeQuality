...
```{r, echo=FALSE,tab.cap="Table X. Species list."}
knitr::kable(dbGetQuery(connection,"SELECT Taxa
, Family
, CommonName
, LifeForm
-- , GrowthHabit
, GrowthHabitSub as GrowthHabit
FROM Summary_VegData_DistinctSpeciesOverall
ORDER BY Taxa"))
```

## Species Diversity By Plot

```{r, echo=FALSE,tab.cap="Table X. Species diversity by plot"}
knitr::kable(dbGetQuery(connection,"SELECT Summary_VegData_DistinctSpeciesByPlot.Plot, Ecotypes.Ecotype, COUNT(DISTINCT Summary_VegData_DistinctSpeciesByPlot.Taxa) AS Species
FROM Summary_VegData_DistinctSpeciesByPlot INNER JOIN
 Plots ON Summary_VegData_DistinctSpeciesByPlot.Plot = Plots.Plot INNER JOIN
 Ecotypes ON Plots.EcotypeID = Ecotypes.EcotypeID
GROUP BY Summary_VegData_DistinctSpeciesByPlot.Plot, Ecotypes.Ecotype
ORDER BY Species DESC"))
```

## Species Diversity By Ecotype

```{r, echo=FALSE,tab.cap="Table X. Species diversity by ecotype."}
DivByEcotype = dbGetQuery(connection,"SELECT Ecotypes.Ecotype, COUNT(DISTINCT Summary_VegData_DistinctSpeciesByPlot.Taxa) AS Species
, AVG(Plots.Elev_m) AS [Mean elevation (m)], STDEV(Plots.Elev_m) AS [Elevation (s.d.)]
FROM Summary_VegData_DistinctSpeciesByPlot INNER JOIN
 Plots ON Summary_VegData_DistinctSpeciesByPlot.Plot = Plots.Plot INNER JOIN
 Ecotypes ON Plots.EcotypeID = Ecotypes.EcotypeID
GROUP BY Ecotypes.Ecotype
...
knitr::kable(DivByEcotype)

ggplot(DivByEcotype,aes(x=Species,y=`Mean elevation (m)`)) +
  geom_point() +
...
knitr::kable(sqldf("SELECT ScientificName,PctCover_TopCanopy as [Foliar % cover] 
FROM CoverData 
...
This may not be useful, few hits. LM ignored these data.
...
knitr::kable(sqldf("SELECT ScientificName,PctCover_LowerInterceptsCombined as [Mid-story % cover] 
FROM CoverData 
WHERE GrowthHabitSub NOT IN ('Moss','Lichen Crust','Abiotic')
...
knitr::kable(sqldf("SELECT ScientificName,PctCover_SoilSurface as [Basal % cover] 
FROM CoverData 
WHERE GrowthHabitSub NOT IN ('Moss','Lichen Crust','Abiotic')
...
# Lichen Height and Biomass

```{r, echo=FALSE,fig.cap="Figure X. Lichen height by ecotype."}
