...
### Lichen thickness ANOVA by ecotype
...
### Basic stats on lichen thicknes

```{r,echo=FALSE}
library(corrplot)
library(psych)

Thickness = Dataset_VegData %>% select(LichenHeight,Ecotype) %>% filter(is.na(LichenHeight)==FALSE)
knitr::kable(t(describe(Thickness$LichenHeight)))

# Plot the dataset
Binwidth <- (max(Thickness$LichenHeight,na.rm=TRUE) - min(Thickness$LichenHeight,na.rm=TRUE)) / ceiling(log2(nrow(Thickness)) )
ggplot(Thickness) + 
  geom_histogram(aes(x=LichenHeight,fill=Ecotype),na.rm=TRUE,binwidth=Binwidth) + 
  theme_minimal() +
  ggtitle("Lichen thickness")

ggplot(Thickness) + 
  geom_boxplot(aes(y=LichenHeight,fill=Ecotype),na.rm=TRUE) + 
  theme_minimal() +
  ggtitle("Lichen thickness")
...
SummarizeThickness = function(Ecotype){
  ...
  cat("#### ",Ecotype,"  \n\n")
  print(knitr::kable(t(describe(Thickness$LichenHeight))))
  cat("  \n\n")
