...
tukeydf = tukeydf %>%
  ...
  mutate(P = round(`p adj`,digits=2)) %>% # Round the p-adj column into a column named P
  ...
  #rename(P = `p adj`)%>% 
  arrange(diff)
tukeydf$Sig = ifelse(tukeydf$P < 0.05,"*","") # Make a column to hold asterisks for significant differences between Ecotype1 and 2
...
```{r,label="Tukey test results plot",fig.cap=paste("Figure ",FigureCounter,". Comparison of Tukey's mean lichen height (cm) between ecotypes.",sep="")}
# Create a ggplot
ggplot(tukeydf) +
  geom_errorbar(aes(y=reorder(EcotypeComparison,-diff),xmin=lwr,xmax=upr),color='gray') +
  geom_point(aes(y=reorder(EcotypeComparison,-diff),x=diff,size=P,color=Sig)) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  geom_text(aes(y=reorder(EcotypeComparison,-diff),x=diff,label=paste(format(P, nsmall = 3),Sig,sep="")),size=3, nudge_x = 0.3, nudge_y = 0.3) +
  theme_minimal() +
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) +
