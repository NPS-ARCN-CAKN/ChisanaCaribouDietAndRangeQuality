...
```
### LichenHeight data checking [not for publication]

Descriptive statistics on lichen height data

```{r,label="LichenHeight descriptive statistics"}

# Descriptive statistics
df = describe(Dataset_VegData$LichenHeight)
df <- data.frame(Statistic = rownames(df), df) 
...
ANOVA and Tukey's test on lichen height by ecotype

These ecotypes are functionally the same lichen thickness
                                                         diff         lwr         upr     p adj
Alpine Sedge-Dwarf Shrub and Alpine Meadow        0.15991516 -0.25270469  0.57253502 0.8781186
Subalpine Wood/Shrub and Alpine Meadow            0.12912649 -0.25371367  0.51196664 0.9290281
Subalpine Wood/Shrub and Alpine Sedge-Dwarf Shrub  -0.03078868 -0.49552937  0.43395202 0.9999664
...
print('ANOVA: LichenHeight ~ Ecotype')
# Perform the ANOVA 
anova_result <- aov(LichenHeight ~ Ecotype, data = Dataset_VegData)
anova_result
Tukey = TukeyHSD(anova_result) 
Tukey
knitr::kable(Tukey$Ecotype)
```

```{r,label="Create mean lichen height by plot dataset}
...
### Lichen Thickness and Biomass by Ecotype
